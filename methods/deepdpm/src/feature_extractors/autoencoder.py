...
class ConvAutoEncoder(nn.Module):
    ...
    def __init__(self, args, input_dim):
        ...
        self.latent_dim = args.latent_dim
        self.encoder_conv = nn.Sequential(UnFlatten(channel=1, width=16), nn.Conv2d(1, 32, 5, stride=1), nn.BatchNorm2d(32), nn.ReLU())
        self.encoder_maxPool = nn.MaxPool2d(2, stride=2, return_indices=True)
        self.encoder_linear = nn.Sequential(Flatten(), nn.Linear(32 * 6 * 6, self.latent_dim))
        self.decoder_linear = nn.Sequential(nn.Linear(self.latent_dim, 32 * 6 * 6), UnFlatten(channel=32, width=6))
        self.decoder_maxPool = nn.MaxUnpool2d(2, stride=2)
        self.decoder_conv = nn.Sequential(nn.ReLU(), nn.BatchNorm2d(32), nn.ConvTranspose2d(32, 1, 5, stride=1), Flatten())

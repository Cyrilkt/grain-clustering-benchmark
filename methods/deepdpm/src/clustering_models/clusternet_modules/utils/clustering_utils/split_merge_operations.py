...
from src.clustering_models.clusternet_modules.utils.clustering_utils.clustering_operations import _create_subclusters, compute_data_covs_soft_assignment, init_mus_and_covs_sub, comp_subclusters_params_min_dist
...
def log_Hastings_ratio_split(alpha, N_k_1, N_k_2, log_ll_k_1, log_ll_k_2, log_ll_k, split_prob):
    ...
    if N_k_2 > 0 and N_k_1 > 0:
        H = np.log(alpha) + lgamma(N_k_1) + log_ll_k_1 + lgamma(N_k_2) + log_ll_k_2 - (lgamma(N_k) + log_ll_k)
...
def log_Hastings_ratio_merge(alpha, N_k_1, N_k_2, log_ll_k_1, log_ll_k_2, log_ll_k, merge_prob):
    ...
    N_k = N_k_1 + N_k_2
    if N_k > 0:
        H = lgamma(N_k) - (np.log(alpha) + lgamma_1 + lgamma_2) + (log_ll_k - (log_ll_k_1 + log_ll_k_2))
    else:
        H = torch.ones(1)
    print(f'Alpha: {alpha}')
    print(f'N_k_1: {N_k_1}')
...
def split_rule(k, codes, logits, logits_sub, mus, mus_sub, cov_const, alpha, split_prob, prior=None, ignore_subclusters=False):
    ...
    N_k_1 = len(codes_k_1)
    N_k_2 = len(codes_k_2)
    return [k, log_Hastings_ratio_split(alpha, N_k_1, N_k_2, log_ll_k_1, log_ll_k_2, log_ll_k, split_prob)]

def compute_split_log_marginal_ll():
    pass

def compute_split_log_ll(mu, mus_sub_1, mus_sub_2, cov_const, codes_k, codes_k_1, codes_k_2):
    D = len(mu)
    dist_k = torch.distributions.multivariate_normal.MultivariateNormal(mu, torch.eye(D) * cov_const)
    dist_k_1 = torch.distributions.multivariate_normal.MultivariateNormal(mus_sub_1, torch.eye(D) * cov_const)
    dist_k_2 = torch.distributions.multivariate_normal.MultivariateNormal(mus_sub_2, torch.eye(D) * cov_const)
...
def split_step(K, codes, logits, logits_sub, mus, mus_sub, cov_const, alpha, split_prob, prior=None, ignore_subclusters=False):
    split_decisions = []
    for k in range(K):
        split_decisions.append(split_rule(k, codes, logits, logits_sub, mus, mus_sub, cov_const, alpha, split_prob, prior=prior, ignore_subclusters=ignore_subclusters))
...
def update_clusters_params_split(mus, covs, pi, mus_ind_to_split, split_decisions, mus_sub, covs_sub, pi_sub):
...
def update_subclusters_params_split(mus_sub, covs_sub, pi_sub, mus_ind_to_split, split_decisions, codes, logits, logits_sub, n_sub, how_to_init_mu_sub, prior, use_priors=True):
    mus_sub_new = mus_sub[torch.logical_not(split_decisions).repeat_interleave(2)]
    covs_sub_new = covs_sub[torch.logical_not(split_decisions).repeat_interleave(2)]
    pi_sub_new = pi_sub[torch.logical_not(split_decisions).repeat_interleave(2)]
    mus_sub_to_add, covs_sub_to_add, pis_sub_to_add = ([], [], [])
    for k in mus_ind_to_split:
        new_mus_sub_1, new_covs_sub_1, new_pis_1 = _create_subclusters(k_sub=2 * k, codes=codes, logits=logits, logits_sub=logits_sub, mus_sub=mus_sub, pi_sub=pi_sub, n_sub=n_sub, how_to_init_mu_sub=how_to_init_mu_sub, prior=prior, use_priors=use_priors)
        new_mus_sub_2, new_covs_sub_2, new_pis_2 = _create_subclusters(k_sub=2 * k + 1, codes=codes, logits=logits, logits_sub=logits_sub, mus_sub=mus_sub, pi_sub=pi_sub, n_sub=n_sub, how_to_init_mu_sub=how_to_init_mu_sub, prior=prior, use_priors=use_priors)
...
def update_models_parameters_split(split_decisions, mus, covs, pi, mus_ind_to_split, mus_sub, covs_sub, pi_sub, codes, logits, logits_sub, n_sub, how_to_init_mu_sub, prior, use_priors):
    mus_ind_to_split = torch.nonzero(split_decisions, as_tuple=False)
    mus_new, covs_new, pi_new = update_clusters_params_split(mus, covs, pi, mus_ind_to_split, split_decisions, mus_sub, covs_sub, pi_sub)
    mus_sub_new, covs_sub_new, pi_sub_new = update_subclusters_params_split(mus_sub, covs_sub, pi_sub, mus_ind_to_split, split_decisions, codes, logits, logits_sub, n_sub, how_to_init_mu_sub, prior, use_priors=use_priors)
    return (mus_new, covs_new, pi_new, mus_sub_new, covs_sub_new, pi_sub_new)

def update_clusters_params_merge(mus_lists_to_merge, inds_to_mask, mus, covs, pi, K, codes, logits, prior, use_priors, n_sub, how_to_init_mu_sub):
    ...
    for pair in mus_lists_to_merge:
        ...
        if N_k > 0:
            ...
            cov_new = compute_data_covs_soft_assignment(logits=(logits[:, pair[0]] + logits[:, pair[1]]).reshape(-1, 1), codes=codes, K=1, mus=mus_mean, prior_name=prior.name)
        else:
...
def update_subclusters_params_merge(mus_lists_to_merge, inds_to_mask, mus, covs, pi, mus_sub, covs_sub, pi_sub, codes, logits, n_sub, how_to_init_mu_sub, prior, use_priors=True):
...
def update_models_parameters_merge(mus_lists_to_merge, inds_to_mask, K, mus, covs, pi, mus_sub, covs_sub, pi_sub, codes, logits, prior, use_priors, n_sub, how_to_init_mu_sub):
    mus_new, covs_new, pi_new = update_clusters_params_merge(mus_lists_to_merge, inds_to_mask, mus, covs, pi, K, codes, logits, prior, use_priors, n_sub, how_to_init_mu_sub)
    mus_sub_new, covs_sub_new, pi_sub_new = update_subclusters_params_merge(mus_lists_to_merge, inds_to_mask, mus, covs, pi, mus_sub, covs_sub, pi_sub, codes, logits, n_sub, how_to_init_mu_sub, prior, use_priors=use_priors)
    return (mus_new, covs_new, pi_new, mus_sub_new, covs_sub_new, pi_sub_new)

def merge_step(mus, logits, codes, K, raise_merge_proposals, cov_const, alpha, merge_prob, h_merge='pairs', prior=None):
    """
    ...
    if raise_merge_proposals == 'kmeans':
        ...
        for i in range(n_cluster):
            ...
            merge_decision, highest_ll = merge_rule(mus, logits, codes, chosen_ind[perm], alpha, cov_const, merge_prob, prior=prior)
            for n_pair in range(len(merge_decision)):
                if merge_decision[n_pair]:
                    mus_to_merge.append([chosen_ind[perm][2 * n_pair:2 * n_pair + 2][0][0], chosen_ind[perm][2 * n_pair:2 * n_pair + 2][1][0]])
                    highest_ll_mus.append(highest_ll[n_pair])
    elif raise_merge_proposals == 'brute_force_NN' or raise_merge_proposals == 'brute_force_NN_with_bad':
        n_neighbors = min(3, K)
        ...
        for proposed_pair in neigh_inds_per_cluster:
            ...
            if p_0 in mus_to_consider_to_merge.keys() and p_1 in mus_to_consider_to_merge.keys():
                merge_decision, highest_ll = merge_rule(mus, logits, codes, proposed_pair, alpha, cov_const, merge_prob, prior=prior)
                if merge_decision[0]:
        ...
        if raise_merge_proposals == 'brute_force_NN_with_bad':
            for i in range(len(mus)):
                ...
                while flag:
                    sampled = torch.randint(len(mus), size=(1,)).item()
                    flag = neighbors_ind[sampled]
                merge_decision, highest_ll = merge_rule(mus, logits, codes, torch.tensor([i, sampled]), alpha, cov_const, merge_prob, prior=prior)
...
def merge_rule(mus, logits, codes, k_inds, alpha, cov_const, merge_prob, prior=None):
    ...
    for i in range(0, len(k_inds), 2):
        ...
        if prior is None:
            log_ll_k, log_ll_k_1, log_ll_k_2 = compute_split_log_ll(mus_mean, mus[k_1], mus[k_2], cov_const, codes_k, codes_k_1, codes_k_2)
        else:

...
class NIW_prior:
    ...
    def compute_params_post(self, codes_k, mu_k):
        ...
        psi_star = self.niw_psi + S + self.niw_kappa * N_k / kappa_star * (mu_k - self.niw_m).unsqueeze(1) @ (mu_k - self.niw_m).unsqueeze(0)
        return (kappa_star, nu_star, mu_0_star, psi_star)

    def compute_post_mus(self, N_ks, data_mus):
        return (N_ks.reshape(-1, 1) * data_mus + self.niw_kappa * self.niw_m) / (N_ks.reshape(-1, 1) + self.niw_kappa)

    def compute_post_cov(self, N_k, mu_k, data_cov_k):
        D = len(mu_k)
        if N_k > 0:
            return (self.niw_psi + data_cov_k * N_k + self.niw_kappa * N_k / (self.niw_kappa + N_k) * ((mu_k - self.niw_m).unsqueeze(1) * (mu_k - self.niw_m).unsqueeze(0))) / (self.niw_nu + N_k + D + 2)
        else:
            return self.niw_psi

    def log_marginal_likelihood(self, codes_k, mu_k):
        kappa_star, nu_star, mu_0_star, psi_star = self.compute_params_post(codes_k, mu_k)
        N_k, D = codes_k.shape
        return -(N_k * D / 2.0) * np.log(np.pi) + mvlgamma(torch.tensor(nu_star / 2.0), D) - mvlgamma(torch.tensor(self.niw_nu) / 2.0, D) + self.niw_nu / 2.0 * torch.logdet(self.niw_psi) - nu_star / 2.0 * torch.logdet(psi_star) + D / 2.0 * (np.log(self.niw_kappa) - np.log(kappa_star))
...
class NIG_prior:
    ...
    def compute_params_post(self, codes_k, mu_k=None):
        N = len(codes_k)
        V_star = self.nig_V * (1.0 / (1 + self.nig_V * N))
        m_star = V_star * (self.nig_m / self.nig_V + codes_k.sum(axis=0))
        a_star = self.nig_a + N / 2.0
        b_star = self.nig_b + 0.5 * (self.nig_m ** 2 / self.nig_V + (codes_k ** 2).sum(axis=0) - m_star ** 2 / V_star)
        return (V_star, m_star, a_star, b_star)

    def compute_post_mus(self, N_ks, data_mus):
        return (N_ks.reshape(-1, 1) * data_mus + 1 / self.nig_V * self.nig_m) / (N_ks.reshape(-1, 1) + 1 / self.nig_V)

    def compute_post_cov(self, N_ks, mus, data_stds):
        if N_ks > 0:
            post_sigma_sq = (data_stds * N_ks + 2 * self.nig_b + 1 / self.nig_V * (self.nig_m - mus) ** 2) / (N_ks + 2 * self.nig_a + 3)
    ...
    def log_marginal_likelihood(self, codes_k, mu_k):
        ...
        for d in range(self.dim):
            lm_ll += 0.5 * (torch.log(torch.abs(V_star[d])) - torch.log(torch.abs(self.nig_V[d]))) + self.nig_a[d] * torch.log(self.nig_b[d]) - a_star[d] * torch.log(b_star[d]) + lgamma(a_star[d]) - lgamma(self.nig_a[d]) - N / 2.0 * torch.log(torch.tensor(np.pi)) - N * torch.log(torch.tensor(2.0))
        return lm_ll

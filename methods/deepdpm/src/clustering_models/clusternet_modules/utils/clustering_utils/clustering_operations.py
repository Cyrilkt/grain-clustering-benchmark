...
def init_mus_and_covs(codes, K, how_to_init_mu, logits, use_priors=True, prior=None, random_state=0, device='cpu'):
    ...
    if how_to_init_mu == 'kmeans':
        ...
        if use_priors:
            ...
            for k in range(K):
                ...
                if not positive_definite.check(cov_k):
                    cov_k = ensure_positive_definite(cov_k, initial_jitter=1e-05, max_iter=10, epsilon=1e-06)
                covs.append(cov_k)
        ...
        kmeans_mus = torch.tensor(pca.inverse_transform(cluster_centers.cpu().numpy()), device=device, requires_grad=False)
        ...
        if use_priors:
            ...
            for k in range(K):
                codes_k = codes[labels == k]
                cov_k = prior.compute_post_cov(counts[k], codes_k.mean(dim=0), data_covs[k])
                if not positive_definite.check(cov_k):
    ...
    if how_to_init_mu_sub == 'kmeans':
        ...
        if use_priors:
            ...
            for k in range(n_sub):
                ...
                if not positive_definite.check(covs_sub_k):
                    covs_sub_k = ensure_positive_definite(covs_sub_k, initial_jitter=1e-05, max_iter=10, epsilon=1e-06)
        ...
        counts.append(c)
        counts = counts[0]
        mus_sub = torch.tensor(pca.inverse_transform(cluster_centers.cpu().numpy()), device=device, requires_grad=False)
        data_covs_sub = compute_data_covs_hard_assignment(labels, codes_k, n_sub, mus_sub, prior)
        if use_priors:
            mus_sub = prior.compute_post_mus(counts, mus_sub.cpu())
            covs_sub = []
            for k in range(n_sub):
                ...
                if not positive_definite.check(cov_sub_k):
                    cov_sub_k = ensure_positive_definite(cov_sub_k, initial_jitter=1e-05, max_iter=10, epsilon=1e-06)
...
def compute_data_covs_hard_assignment(labels, codes, K, mus, prior):
    if prior and prior.mus_covs_prior.name == 'NIG':
        ...
        for k in range(K):
            ...
            if N_k > 0:
                cov_k = torch.matmul((codes_k - mus[k].cpu().repeat(len(codes_k), 1)).T, codes_k - mus[k].cpu().repeat(len(codes_k), 1))
                cov_k = cov_k / N_k
            elif prior:
...
def compute_data_sigma_sq_soft_assignment(codes, logits, K, mus):
    ...
    stds = torch.stack([(logits[:, k].unsqueeze(1) * (codes - mus[k]) ** 2).sum(axis=0) / denominator[k] for k in range(K)])
    return stds

def compute_mus_soft_assignment(codes, logits, K, constant=True):
    denominator = logits.sum(axis=0)
    mus = torch.stack([(logits[:, k].reshape(-1, 1) * codes).sum(axis=0) / denominator[k] for k in range(K)])
    if constant:
        mus = mus.detach()
    return mus
    ...
    if prior_name == 'NIW':
        ...
        for k in range(K):
            if len(logits) == 0 or len(codes) == 0:
                ...
                cov_k = torch.matmul(logits[:, k] * (codes - mus[k].repeat(len(codes), 1)).T, codes - mus[k].repeat(len(codes), 1))
                cov_k = cov_k / n_k[k]
            covs.append(cov_k)
        return torch.stack(covs)
    elif prior_name == 'NIG':
        ...
        for k in range(K):
            ...
            if indices.sum() < 2 or denominator[0] == 0 or denominator[1] == 0 or (len(torch.unique(r_sub.argmax(-1))) < n_sub):
                ...
                for k_sub in range(n_sub):
                    z_sub = r_sub[:, k_sub]
                    mus_sub_k.append((z_sub.reshape(-1, 1) * codes_k.cpu()).sum(axis=0) / denominator[k_sub])
...
def compute_mus_subclusters(codes, logits, logits_sub, pi_sub, mus_sub, K, n_sub, hard_assignment=True, use_priors=True, prior=None):
    if hard_assignment:
        ...
        for k in range(K):
            ...
            indices = logits.argmax(-1) == k
            if indices.sum() < 5:
                mus_sub_new.append(mus_sub[2 * k:2 * k + 2].clone().detach().cpu().type(torch.float32))
            else:
                codes_k = codes[indices]
                for k_sub in range(n_sub):
                    if denominator[k_sub] == 0:
                        mus_sub_new.append(mus_sub[2 * k + k_sub].clone().detach().cpu().type(torch.float32).unsqueeze(0))
                    else:
                        z_sub = logits_sub[indices, 2 * k + k_sub]
                        mus_sub_new.append(((z_sub.reshape(-1, 1) * codes_k.cpu()).sum(axis=0) / denominator[k_sub]).unsqueeze(0))
...
def _create_subclusters(k_sub, codes, logits, logits_sub, mus_sub, pi_sub, n_sub, how_to_init_mu_sub, prior, device=None, random_state=0, use_priors=True):
    ...
    if how_to_init_mu_sub == 'soft_assign':
        ...
        if how_to_init_mu_sub == 'kmeans':
            ...
            device = 'cuda' if torch.cuda.is_available() else 'cpu'
            labels, cluster_centers = GPU_KMeans(X=torch.from_numpy(pca_codes), num_clusters=n_sub, device=torch.device('cpu'))
            new_mus = torch.tensor(pca.inverse_transform(cluster_centers.cpu().numpy()), device=device, requires_grad=False).cpu()
            new_covs = compute_data_covs_hard_assignment(labels=labels, codes=codes_sub, K=n_sub, mus=new_mus, prior=prior)
            _, new_pis = torch.unique(labels, return_counts=True)
            new_pis = new_pis / float(len(codes_sub)) * pi_sub[k_sub]
        if use_priors:
            ...
            for k in range(n_sub):
                ...
                if not positive_definite.check(new_cov_k):
                    new_cov_k = ensure_positive_definite(new_cov_k)

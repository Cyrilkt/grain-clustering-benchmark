...
class MLP_Classifier(nn.Module):
    ...
    def update_K_split(self, split_decisions, init_new_weights='same', subclusters_nets=None):
        ...
        with torch.no_grad():
            ...
            new_weights = self._initalize_weights_split(weights_splits, split_decisions, init_new_weight=init_new_weights, subclusters_nets=subclusters_nets)
            self.class_fc2.weight.data = torch.cat([weights_not_split, new_weights])
    ...
    def update_K_merge(self, merge_decisions, pairs_to_merge, highest_ll, init_new_weights='same'):
        ...
        with torch.no_grad():
            ...
            new_weights = self._initalize_weights_merge(weights_merged, merge_decisions, highest_ll, init_new_weight=init_new_weights)
            class_fc2.weight.data = torch.cat([weights_not_merged, new_weights])
    ...
    def _initalize_weights_split(self, weight, split_decisions, init_new_weight, subclusters_nets=None):
        if init_new_weight == 'same':
            ...
            return torch.FloatTensor(weight.shape[0] * 2, weight.shape[1]).uniform_(-1.0, 1).to(device=self.device)
    ...
    def _initalize_weights_merge(self, weights_merged, merge_decisions, highest_ll, init_new_weight='same'):
        if init_new_weight == 'same':
            ...
            return torch.FloatTensor(len(highest_ll), weights_merged.shape[1]).uniform_(-1.0, 1).to(device=self.device)
...
class Subclustering_net(nn.Module):
    ...
    def update_K_split(self, split_decisions, init_new_weights='same'):
        ...
        with torch.no_grad():
            ...
            fc1_new_weights = self._initalize_weights_split(fc1_weights_split, init_new_weight=init_new_weights)
            self.class_fc1.weight.data = torch.cat([fc1_weights_not_split, fc1_new_weights])
    ...
    def update_K_merge(self, merge_decisions, pairs_to_merge, highest_ll, init_new_weights='highest_ll'):
        ...
        with torch.no_grad():
            ...
            for merge_pair, highest_ll_k in zip(pairs_to_merge, highest_ll):
                fc1_weights_merged = [class_fc1.weight.data[k * self.hidden_dim:(k + 1) * self.hidden_dim, :] for k in merge_pair]
                fc1_new_weights.append(self._initalize_weights_merge(fc1_weights_merged, (torch.tensor(highest_ll_k) == merge_pair[1]).item(), init_new_weight=init_new_weights))
                fc1_bias_merged = [class_fc1.bias.data[k * self.hidden_dim:(k + 1) * self.hidden_dim] for k in merge_pair]
                fc1_new_bias.append(self._initalize_weights_merge(fc1_bias_merged, (torch.tensor(highest_ll_k) == merge_pair[1]).item(), init_new_weight=init_new_weights))
            fc1_new_weights = torch.cat(fc1_new_weights)
            fc1_new_bias = torch.cat(fc1_new_bias)
            self.class_fc1.weight.data = torch.cat([fc1_weights_not_merged, fc1_new_weights])
            self.class_fc2.weight.data.fill_(0)
            gradient_mask_fc2 = torch.zeros(self.hidden_dim * self.K, 2 * self.K)
    ...
    def _initalize_weights_split(self, weight, init_new_weight, num=2):
        if init_new_weight == 'same':
            ...
            return torch.FloatTensor(weight.shape[0] * num, weight.shape[1]).uniform_(-1.0, 1).to(device=self.device)
        else:
            raise NotImplementedError
    ...
    def _initalize_weights_merge(self, weights_list, highest_ll_loc, init_new_weight='highest_ll', num=2):
        if init_new_weight == 'highest_ll':
            ...
            return torch.FloatTensor(weights_list[0].shape[0], weights_list[0].shape[1]).uniform_(-1.0, 1).to(device=self.device)
        else:
            raise NotImplementedError
